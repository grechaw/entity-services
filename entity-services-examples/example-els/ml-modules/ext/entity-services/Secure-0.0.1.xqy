...
(:
 ...
 The source model was Secure-0.0.1
 ...
 Generated at timestamp: 2017-09-08T21:50:00.808797Z
 :)

module namespace secure
    = 'http://marklogic.com/entity-services/example-els/Secure-0.0.1';
...
(:~
 ...
 :)
declare function secure:extract-instance-Customer(
    $source as item()?
) as map:map
{
...
(:~
 ...
 :)
declare function secure:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( secure:canonicalize($entity-instance) )/node()
        else secure:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function secure:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, secure:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, secure:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function secure:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { secure:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    secure:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function secure:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := secure:instance-to-canonical($entity-instance, $envelope-format)
...
{
    secure:instance-to-envelope($entity-instance, "xml")
};
