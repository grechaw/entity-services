...
public class ExamplesLoader extends ExamplesBase {
    ...
    public Thread modelsLoad() {
        Runnable task = () -> {
            try {
                importJSON(Paths.get(props.getProperty("entityTypesDir")), "http://marklogic.com/entity-services/models");
            } catch (IOException e) {
    ...
    public void harmonize() throws InterruptedException {
        ...
        ApplyTransformListener listener =
                new ApplyTransformListener()
                        .withTransform(ingester)
                        .withApplyResult(ApplyTransformListener.ApplyResult.IGNORE)
                        .onSuccess( (dbClient, inPlaceBatch) -> {
                            logger.info("SUCCESS");
                        })
                        .onBatchFailure(
                                (dbClient, inPlaceBatch, throwable) -> {
                                    logger.warn("FAILURE on batch:" + inPlaceBatch.toString() + "\n",
                                            throwable);
                                    throwable.printStackTrace();
                                });

        QueryHostBatcher queryHostBatcher = moveMgr.newQueryHostBatcher(qb.build())
                .withBatchSize(100)
                .withThreadCount(5)
